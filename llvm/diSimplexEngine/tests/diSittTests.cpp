...
describe(DiSITT) {
  ...
  pending_it("should be able to create a simplex from an empty collection of simplicies") {
    DiSITT universe = DiSITT::getUniverse();
    shouldNotBeNULL(universe.ref);
    DiStructure initial = universe.getInitialStructure();
    shouldNotBeNULL(initial.ref);
...
//    shouldBeEqual(simplex0.getLabel().ref, initial.ref);
  ...
  pending_it("should be able to create a simplex from a collection of one simplex") {
    DiSITT universe = DiSITT::getUniverse();
    shouldNotBeNULL(universe.ref);
    DiStructure initial = universe.getInitialStructure();
    shouldNotBeNULL(initial.ref);
...
//    shouldBeEqual(simplex0a.id, simplex0.id);
  ...
  pending_it("should be able to create a simplex from a collection of two simplicies") {
    DiSITT universe = DiSITT::getUniverse();
    shouldNotBeNULL(universe.ref);
    DiStructure initial = universe.getInitialStructure();
    shouldNotBeNULL(initial.ref);
